...
MODE = os.environ.get('COAX_ABS_MODE', 'MUR')  # 'MUR' (Modal Mur) or 'SCALAR' (Zw splitter)
...
# PEC on the propagation faces for BOTH methods. A MUR face behind the sheet
# absorbs on its own, so a scalar sheet in front of one is not measured in
# isolation -- that is what made the CPW splitter look 17 dB better than it is.
# The transverse faces are irrelevant here: the shield encloses the field.
