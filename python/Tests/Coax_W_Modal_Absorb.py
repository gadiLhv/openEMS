...
Zz = mesh.GetLines('z')
# Absorber 1 must sit BEHIND the excitation (lower z), several cells away:
# the port excitation is at the port START plane and the ut/it probes at the
# port STOP plane. Placing the absorber sheet at idxPort1+1 puts it exactly on
# the probe plane, with its H-sample/injection dual cell straddling the
# excitation plane -> active feedback loop with the source (diverges).
idxPort1 = (np.where(Zz == 0.0)[0] + 3).item(0)
idxPort2 = (np.where(Zz == coax_L)[0] - 3).item(0)
idxAbs1  = idxPort1 - 2   # behind the excitation → catches the backward launch
idxAbs2  = idxPort2 + 2   # one cell outside the second port plane → absorber location
...
# --- Port 2: passive waveguide port (S21 measurement only, same as Coax_W_WG_Ports.py) ---
# Reversed start/stop so the port faces the incoming +z wave; its probe planes
# stay clear of the absorber sheet one cell beyond idxPort2.
...
# ## Post-processing
# Port 1 measures the reflection off the absorber (S11); the passive port 2
# measures the transmission up to the absorber plane (S21, expected ~0 dB).
...
plot(f / 1e9, s21_dB, 'b-', linewidth=2, label='$S_{21}$ (transmission to far-end port plane)')
grid()
legend()
